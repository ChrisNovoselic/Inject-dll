...
static std::wstring gwsProcName = L"explorer.exe"; // ingrone character case
...
INT WINAPI StartHook()
{
	INT codeRet = 0;

	if (!Hook::Run("ntdll.dll","NtQuerySystemInformation", NewNtQuerySystemInformation, reinterpret_cast<void **>(&RealNtQuerySystemInformationPtr)))
	{
...
INT WINAPI StopHook()
{
	...
	{
		Hook::Run("ntdll.dll", "NtQuerySystemInformation", RealNtQuerySystemInformationPtr, reinterpret_cast<void **>(&RealNtQuerySystemInformationPtr));
...
BOOL WINAPI DllMain(_In_ HINSTANCE, _In_ DWORD fdwReason, _In_ LPVOID lpszReserved)
{
	switch (fdwReason)
	{
