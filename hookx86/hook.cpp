...
BOOL Hook::InlineHook(_In_ LPVOID HookProcPtr, _In_ LPVOID NewProcPtr, _Out_ LPVOID* RealProcPtr)
{
#define JMP_SIZE 0x5
	DWORD dwPatchSize	= 0;
	LPVOID lpHookFunc	= nullptr;
	DWORD dwBytesNeed	= 0;
	LPBYTE lpPatchBuffer = nullptr;
	DWORD dwOldProectd	= 0;


	if (HookProcPtr == nullptr || NewProcPtr == nullptr || RealProcPtr == nullptr)
	{
		return FALSE;
	}


	if (!GetPatchSize(HookProcPtr, JMP_SIZE, &dwPatchSize))
	{
		return FALSE;
	}
	...
	*(DWORD *)lpHookFunc = dwPatchSize;
	...
	memcpy((BYTE *)lpHookFunc + JMP_SIZE, HookProcPtr, dwPatchSize);

	lpPatchBuffer = (LPBYTE)VirtualAlloc(NULL, dwPatchSize, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	...
	*(DWORD*)((DWORD)lpHookFunc + 1) = (DWORD)NewProcPtr - (DWORD)lpHookFunc - JMP_SIZE;


	*(BYTE *)((DWORD)lpHookFunc + 5 + dwPatchSize) = 0xE9;
	*(DWORD*)((DWORD)lpHookFunc + 5 + dwPatchSize + 1) = ((DWORD)HookProcPtr + dwPatchSize) - ((DWORD)lpHookFunc + JMP_SIZE + dwPatchSize) - JMP_SIZE;


	//jmp 
	*(BYTE *)lpPatchBuffer = 0xE9;
	*(DWORD*)(lpPatchBuffer + 1) = (DWORD)lpHookFunc - (DWORD)HookProcPtr - JMP_SIZE;


	if (!::VirtualProtect(HookProcPtr, dwPatchSize, PAGE_EXECUTE_READWRITE, &dwOldProectd))
	{
		return FALSE;
	}

	memcpy(HookProcPtr, lpPatchBuffer, dwPatchSize);


	::VirtualProtect(HookProcPtr, dwPatchSize, dwOldProectd, &dwOldProectd);
	::VirtualFree(lpPatchBuffer, 0, MEM_RELEASE);

	*RealProcPtr = (void *)((DWORD)lpHookFunc + JMP_SIZE);
	return TRUE;
}

VOID Hook::UnInlineHook(_In_ LPVOID HookProcPtr, LPVOID RealProcPtr)
{
	DWORD dwOldProectd	= 0;
	DWORD dwPatchSize	= 0;
	LPBYTE lpBuffer		= nullptr;

	lpBuffer	= (LPBYTE)((DWORD)RealProcPtr - (sizeof(DWORD) + JMP_SIZE));
	...
	memcpy(HookProcPtr, RealProcPtr, dwPatchSize);
...
BOOL Hook::GetPatchSize(_In_ LPVOID HookProcPtr, _In_ DWORD dwSize, _Out_ DWORD* pdwPatchSize)
{
	DWORD Length;
	PUCHAR pOpcode;
	DWORD PatchSize = 0;

	if (HookProcPtr == nullptr || pdwPatchSize == nullptr)
	{
		return FALSE;
	}
	...
	do
	{
		Length = SizeOfCode(HookProcPtr, &pOpcode);
		if ((Length == 1) && (*pOpcode == 0xC3))
			break;
		if ((Length == 3) && (*pOpcode == 0xC2))
			break;
		HookProcPtr = (PVOID)((DWORD)HookProcPtr + Length);
	...
	*pdwPatchSize = PatchSize;
	return TRUE;
}

ULONG __fastcall Hook::SizeOfCode(_In_ LPVOID Code, UCHAR ** pOpCode)
{
#define OP_NONE           0x00
